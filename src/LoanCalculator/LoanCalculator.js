...
const LoanCalculator = () => {
  ...
  const [loanTermUnit, setLoanTermUnit] = useState('months'); // Default to months
  ...
  const exportToExcel = () => {
    ...
    const totalPayments = Math.ceil(parseFloat(loanTerm) * 12); // Ensure totalPayments is a whole number
    let remainingPayments = totalPayments;

    // Split loan term into years and months
    ...
    for (let yearIndex = 0; yearIndex < years + 1; yearIndex++) {
      ...
      // Adjust the loop to start from the selected start month
      for (let monthIndex = startMonthIndex - 1; monthIndex < maxMonths; monthIndex++) {
        ...
        if (remainingPayments > 0 && remainingPrincipal > 0.1) {
          ...
          if (remainingPrincipal <= 0.1) {
            // If remaining principal is less than or equal to 0.1, exit the loop
            break;
          }
        } else {
          break;
    ...
    // Setting extra data to display on the front page
    const extraDataString = `Total Payments: ₹${numeral(totalPayments).format('0,0')} | Start Month: ${getMonthName(parseInt(startMonth))}`;
